...
data:
  ...
  code: "#ifndef HARUILIB_MATH_BIG_INTEGER_HPP\n#define HARUILIB_MATH_BIG_INTEGER_HPP\n\
    \n#include <algorithm>\n#include <cassert>\n#include <cctype>\n#include <cstdint>\n\
    #include <iomanip>\n#include <iostream>\n#include <string>\n#include <unordered_map>\n\
    #include <utility>\n#include <vector>\n\nnamespace big_integer_internal {\n\n\
    using ll = long long;\n\ntemplate<int MOD, int G>\nstruct NTT {\n  static int\
    ...
    \      if(i < j) std::swap(a[i], a[j]);\n    }\n\n    for(int len=2; len<=n; len<<=1)\
    \ {\n      int wlen = mod_pow(G, (MOD - 1) / len);\n      if(inverse) wlen = mod_pow(wlen,\
    \ MOD - 2);\n      for(int i=0; i<n; i+=len) {\n        long long w = 1;\n   \
    \     for(int j=0; j<len/2; j++) {\n          int u = a[i + j];\n          int\
    \ v = (int)(w * a[i + j + len / 2] % MOD);\n          int x = u + v; if(x >= MOD)\
    \ x -= MOD;\n          int y = u - v; if(y < 0) y += MOD;\n          a[i + j]\
    \ = x;\n          a[i + j + len / 2] = y;\n          w = w * wlen % MOD;\n   \
    \     }\n      }\n    }\n\n    if(inverse) {\n      int inv_n = mod_pow(n, MOD\
    ...
    \ == '-') sgn = -1;\n      pos++;\n    }\n    int end = pos;\n    while(end <\
    \ (int)s.size() && std::isdigit((unsigned char)s[end])) end++;\n    std::vector<int>\
    \ dec;\n    for(int i=end; i>pos; i-=3) {\n      int l = std::max(pos, i - 3);\n\
